...
func (vt Voting) Size() uint64 {
	size := types.UUID_SIZE + uint64(8) + uint64(len(vt.Title)) + uint64(8) + uint64(8) + uint64(8)
...
func (vt Voting) Marshal() []byte {
	...
	// uuid
	copy(bytes[last_index:last_index+vt.Uuid.Size()], vt.Uuid[:])
	last_index += vt.Uuid.Size()
	// title length
...
func (vt *Voting) Unmarshal(bytes []byte) error {
	...
	// uuid
	copy(vt.Uuid[:], bytes[last_index:last_index+types.UUID_SIZE])
