...
func (op Option) Size() uint64 {
	return types.UUID_SIZE + uint64(8) + uint64(len(op.Title))
}
...
func (op Option) Marshal() []byte {
	...
	// uuid
	copy(bytes[last_index:last_index+types.UUID_SIZE], op.Uuid[:])
...
func (op *Option) Unmarshal(bytes []byte) error {
	...
	// uuid3]
	copy(op.Uuid[:], bytes[last_index:last_index+types.UUID_SIZE])
	last_index += types.UUID_SIZE
	// title legth
