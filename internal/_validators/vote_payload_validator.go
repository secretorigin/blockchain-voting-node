...
func (vl VotePayloadValidator) ValidateVote(vote votings.Vote, votingUuid uuid.UUID) (bool, error) {
	bytes := vote.Marshal()

	ok, err := vl.validator.Validate(
		vote.UserUuid,
		votingUuid,
		bytes[:votings.VOTE_SIZE-types.SIGNATURE_SIZE-1],
		types.Signature(bytes[votings.VOTE_SIZE-types.SIGNATURE_SIZE:]),
	)
	if err != nil {
		return false, err
	}
...
func (vl VotePayloadValidator) ValidateNode(node blockchains.Node, votingUuid uuid.UUID) (bool, error) {
	bytes := node.Marshal()
